...
const LoginCareseeker = ({ login, isAuthenticated }) => {
  ...
  if (isAuthenticated) {
    return <Redirect to="/profile-careseeker" />;
  }
  ...
  return (
    <Fragment>
      <div className="bg-info text-center mx-auto my-2 w-75 mb-5 mt-5 rounded pt-5">
        ...
        <p className="pb-4">
          Not Registered yet? <Link to="/signup-careseeker">Sign up</Link>
        </p>
      </div>
    </Fragment>
  );
...
export default connect(mapStateToProps, { login })(LoginCareseeker);
